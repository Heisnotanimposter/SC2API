...
from sc2.constants import NEXUS, PROBE, PYLON, ASSIMILATOR, GATEWAY, CYBERNETICSCORE, STARGATE, VOIDRAY, ROBOTICSFACILITY, OBSERVER
import random
import cv2
import numpy as np
import os
...
class testapi1(sc2.BotAI):
    def init(self, use_model=False):
        ...
        if self.use_model:
            print("USING MODEL!")
            self.model = keras.models.load_model("train.py")
        ...
        if len(self.units(VOIDRAY).idle) > 0:
            ...
            if self.iteration > self.do_something_after:
                if self.use_model:
                    ...
                    choice_dict = {0: "No Attack!", 1: "Attack close to our nexus!", 2: "Attack Enemy Structure!", 3: "Attack Enemy Start!"}
...
for i in range(100):
    run_game(maps.get("Simple64"), [Bot(Race.Protoss, testapi1(use_model=True)), Computer(Race.Protoss, Difficulty.Medium)], realtime=False)
1